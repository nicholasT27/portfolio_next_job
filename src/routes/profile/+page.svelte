...
<script>
  ...
  async function updateUserData(evt) {
    ...
    const resp = await fetch(PUBLIC_BACKEND_BASE_URL + 'api/collections/users/records/' + `${authData.userId}`, {
            method: 'PATCH',
            mode: 'cors',
            headers: {
...
<div class="flex h-screen w-screen">
      ...
      <div id="flip-card-inner" class="flip-card-inner">
        ...
        <!-- Profile detail -->
         <!-- If loginStatus is true, show the warning message -->
            {#if $getError}
            <div class="warningMessageSlideRight z-10 flex absolute top-2 right-0 items-center p-4 text-yellow-800 rounded-lg bg-yellow-50 dark:bg-gray-800 dark:text-red-400" role="alert">
          ...
          <button on:click={flip} class="mr-2 text-2xl sm:text-lg sm:block md:hidden h-20 w-24 absolute left-3 justify-center items-center">
            <div class="flex flex-row">
              <svg class="w-3 h-3 mt-3 mr-2 text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 8 14">
          ...
          {#if $showImage}
            <img class="w-40 h-40 p-1 rounded-full ring-1 ring-gray-300" src="{userPic}" alt="Bordered avatar">
			    {:else}
        ...
        <label for="file-upload" class="mt-5 rounded-md bg-white px-2.5 py-1.5 text-base sm:text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50">
        ...
        <div class="p-2 m-2 flex flex-col sm:flex-row gap-1 mr-2">
          <div class="flex flex-row mb-2 sm:mb-0">
            <span class="userData p-2 text-white rounded-lg text-center w-24 text-lg sm:text-base" name="username">Username</span>
            <svg class="ml-1 w-6 h-6 text-gray-800 mt-2 mr-10" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 16">
        ...
        <div class="mt-1 flex flex-col">
          <button type="submit" class="rounded-lg w-10 h-10 text-center flex items-center justify-center bg-white p-2 my-2 mr-2 mt-3 sm:mt-0">
              <svg class="w-6 h-6 text-emerald-600" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 16 12">
                  <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M1 5.917 5.724 10.5 15 1.5"/>
              </svg>
          </button>

          <button on:click={close} class="rounded-lg w-10 h-10 text-center flex items-center justify-center bg-white p-2 my-2">
        ...
        {:else}
        <button on:click={usernameEdit} class="rounded-lg w-10 h-10 text-center flex items-center justify-center bg-white p-2 my-4">
        ...
        <div class="p-1 mr-10 flex flex-col sm:flex-row gap-1">
          <div class="flex flex-row">
            <span class="userData p-2 text-white rounded-lg text-center w-24 mr-1 text-lg sm:text-base">Email</span> 
    ...
    <div class="w-1/2 flex justify-center bg-gradient-to-br from-pink-500 to-orange-400 formDivision">
      ...
      {#if $getError}
            <div class="warningMessageSlideRight flex absolute top-2 left-2 items-center p-4 text-yellow-800 rounded-lg bg-yellow-50 dark:bg-gray-800 dark:text-red-400" role="alert">
                <svg class="flex-shrink-0 w-4 h-4" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
                <path d="M10 .5a9.5 9.5 0 1 0 9.5 9.5A9.51 9.51 0 0 0 10 .5ZM9.5 4a1.5 1.5 0 1 1 0 3 1.5 1.5 0 0 1 0-3ZM12 15H8a1 1 0 0 1 0-2h1v-3H8a1 1 0 0 1 0-2h2a1 1 0 0 1 1 1v4h1a1 1 0 0 1 0 2Z"/>
                </svg>
                <span class="sr-only">Info</span>
        ...
        <p class="p-2 m-2 flex flex-row gap-1">
            <span class="userData p-2 text-white rounded-lg text-center w-24" name="username">Username</span>
        ...
        {#if $editForUsername == true}
          <button type="submit" class="border rounded-lg w-10 h-10 text-center flex items-center justify-center bg-white p-2 my-4 mr-2">
              <svg class="w-6 h-6 text-emerald-600" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 16 12">
                  <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M1 5.917 5.724 10.5 15 1.5"/>
              </svg>
          </button>

          <button on:click={close} class="border rounded-lg w-10 h-10 text-center flex items-center justify-center bg-white p-2 my-4">
              <svg class="w-5 h-5 text-red-500" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                  <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6"/>
              </svg>
          </button>
        {:else}
        <button on:click={usernameEdit} class="border rounded-lg w-10 h-10 text-center flex items-center justify-center bg-white p-2 my-4">
              <svg class="w-6 h-6 text-emerald-600" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 18">
                <path d="M12.687 14.408a3.01 3.01 0 0 1-1.533.821l-3.566.713a3 3 0 0 1-3.53-3.53l.713-3.566a3.01 3.01 0 0 1 .821-1.533L10.905 2H2.167A2.169 2.169 0 0 0 0 4.167v11.666A2.169 2.169 0 0 0 2.167 18h11.666A2.169 2.169 0 0 0 16 15.833V11.1l-3.313 3.308Zm5.53-9.065.546-.546a2.518 2.518 0 0 0 0-3.56 2.576 2.576 0 0 0-3.559 0l-.547.547 3.56 3.56Z"/>
                <path d="M13.243 3.2 7.359 9.081a.5.5 0 0 0-.136.256L6.51 12.9a.5.5 0 0 0 .59.59l3.566-.713a.5.5 0 0 0 .255-.136L16.8 6.757 13.243 3.2Z"/>
              </svg>
